...
// 					value='" + item.item_id + "' following='" + 
// 					"<c:forEach var='relation' items='${item.follow_items}'><c:if test='${relation.member_id.member_no eq user.member_no}'><c:if test='${relation.status == 1}'>1</c:if></c:if></c:forEach>"
...
// 					</c:forEach>
					...
					var itemFollow_a = $("<a></a>").attr("value", item.item_id).append(itemFollow_img);
					var itemFollow_div = $("<div class='item_follow_div'></div>").append(itemFollow_a);
